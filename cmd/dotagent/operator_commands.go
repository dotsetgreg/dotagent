...
func newInitCommand(instanceID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Println("     or use local Ollama:")
			fmt.Println("     dotagent config set agents.defaults.provider '\"ollama\"'")
			fmt.Println("     dotagent config set agents.defaults.model '\"llama3.2\"'")
			fmt.Println("     dotagent config set providers.ollama.api_base '\"http://127.0.0.1:11434/v1\"'")
