...
func Run(opts Options) (*Result, error) {
	...
	dotAgentHome, err := resolveDotAgentHome(opts.DotAgentHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, dotAgentHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to DotAgent")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, dotAgentHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, dotAgentHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(dotAgentHome, "config.json"),
				Description: "convert OpenClaw config to DotAgent format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(dotAgentHome)
...
func Execute(actions []Action, openclawHome, dotAgentHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, dotAgentHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, dotAgentHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing DotAgent config: %w", err)
		}
...
func resolveDotAgentHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("DOTAGENT_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".dotagent"), nil
}
