...
func augmentProviderError(providerName, message string) string {
	...
	case ProviderOpenAICodex:
		...
		if strings.Contains(lower, "unsupported parameter: previous_response_id") {
			return msg + " Hint: Codex backend does not accept previous_response_id; update to a build that strips provider state IDs for openai-codex requests."
		}
